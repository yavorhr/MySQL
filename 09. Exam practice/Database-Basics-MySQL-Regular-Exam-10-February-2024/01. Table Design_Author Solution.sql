...
SELECT CONCAT(name, ' is in South Hemisphere'), latitude, longitude, area * 0.39 AS sq_miles, REVERSE(type), established_on
FROM preserves
WHERE latitude < 0;
...
# 4: Delete
DELETE p FROM preserves p
WHERE p.established_on IS NULL;

# 5: Most experienced workers  
    SELECT CONCAT(first_name, ' ', last_name) AS full_name,
    TIMESTAMPDIFF(DAY, start_date, CURDATE()) AS days_of_experience
FROM
    workers
WHERE
    TIMESTAMPDIFF(YEAR, start_date, CURDATE()) > 5
    ORDER BY days_of_experience desc
    LIMIT 10;
    
# 6: Workers salary
SELECT w.id, w.first_name, w.last_name, p.name AS preserve_name 
FROM workers w
...
SELECT  p.name, COUNT(w.is_armed = 1) AS armed_workers
FROM preserves p
...
SELECT p.name, udf_average_salary_by_position_name('Ecologist') as position_average_salary FROM positions p 
WHERE p.name = 'Ecologist';

SELECT p.name, udf_average_salary_by_position_name('Botanist') as position_average_salary FROM positions p 
WHERE p.name = 'Botanist';
...
WHERE p.name = 'Forester';
 ...
 # 11: udp
DELIMITER %%     
 CREATE PROCEDURE udp_increase_salaries_by_country(country_name VARCHAR(255))
 BEGIN
       UPDATE workers w
...
CALL udp_increase_salaries_by_country('Brazil');
...
CALL udp_increase_salaries_by_country('Iceland');
